...
#		fi
		...
		# add enable /dev/ttyS1 
		sed -e 's/exit 0//g' -i /etc/rc.local
		echo "su -c 'hciattach /dev/ttyS1 any'" >> /etc/rc.local
	...
	mkdir -p $destination/etc/skel/Desktop/
	cp -R $SRC/packages/blobs/desktop/skel/. $destination/etc/skel
	if [[ $DE != icewm  ]]; then
	    cp $SRC/cache/SRC/Test* $destination/etc/skel/Desktop
	fi
	...
	# install wallpapers
	mkdir -p $destination/usr/share/backgrounds/$BUILD_DESKTOP_DE/
	cp $SRC/packages/blobs/desktop/wallpapers/armbian*.jpg $destination/usr/share/backgrounds/$BUILD_DESKTOP_DE/
